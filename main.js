...
function projects(){
    ...
    IB_KNN = createA('./letter_classification_IB_KNN.pdf', 'Paper on Iterative Bayes and K-Nearest Neighbors<br>' +
...
function emailBox()
{
    ...
    {
        greeting.position(width1 * 0.25, _height * 0.75 + imgHeight);
        sender.position(width1 * 0.25, _height * 0.75 + imgHeight + eleFont * 2);
        input.position(width1 * 0.25, _height * 0.8 + imgHeight + eleFont * 4);
        button.position(width1 * 0.25 + (eleFont/3)*(sender.value().length), _height * 0.695 + imgHeight + eleFont * 2 + _width/100);
    }
    textAlign(CENTER);
}
